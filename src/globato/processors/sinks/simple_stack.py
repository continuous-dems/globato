...
class SimpleStack(FetchHook):
        ...
        for mod, entry in entries:
            ...
            if not self._global_mode:
                ...
                if self.plug:
                    ...
                    # Wrap stream -> Finalize at end of stream
                    entry['stream'] = self._intercept_per_file(stream, acc)
                    entry['dst_fn'] = out_fn # Optimistically update filename
                    new_entries.append((mod, entry))
                ...
                if self.plug:
                    # Consume into global accumulator
                    self._consume_stream(stream, self._accumulator)
                    # We 'eat' the entry here. Will generate a single result later.
                else:
                    # Pass-through
                    entry['stream'] = self._intercept(stream, entry)
                    new_entries.append((mod, entry))

        # Handle Reduction (Global + Plug)
        if self._global_mode and self.plug and processed_count > 0:
            # We consumed all streams into self._accumulator.
            # Now finalize and return a SINGLE entry representing the grid.
            if self._accumulator:
                self._accumulator.finalize()

                # Create synthetic entry using the first module context
                mod, _ = entries[0]
                stack_entry = {
                ...
                # Clear accumulator to prevent double-finalization
                self._accumulator = None
    ...
    def _intercept_per_file(self, stream, accumulator):
        ...
        for chunk in stream:
            accumulator.add_points(chunk)
            yield chunk
        # Finalize immediately when this stream ends
